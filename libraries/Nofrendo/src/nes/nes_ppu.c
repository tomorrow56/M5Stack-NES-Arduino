...
static void ppu_renderbg(uint8 *vidbuf)
{
   ...
   {
      /* バッファの有効性を確認 */
      if (vidbuf != NULL && ((uint32_t)vidbuf >= 0x3F000000 && (uint32_t)vidbuf < 0x40000000)) {
         memset(vidbuf, FULLBG, NES_SCREEN_WIDTH);
      } else {
         ESP_LOGE(TAG, "Invalid video buffer address: %p", vidbuf);
      }
      return;
   }
...
static void ppu_renderscanline(bitmap_t *bmp, int scanline, bool draw_flag)
{
   ...
   if (bmp == NULL) {
      ESP_LOGE(TAG, "Invalid bitmap (NULL) in ppu_renderscanline");
      return;
   }

   /* Check scanline range */
   if (scanline < 0 || scanline > bmp->height) {
      ESP_LOGE(TAG, "Invalid scanline in ppu_renderscanline: %d (height: %d)", scanline, bmp->height);
      return;
   }
   
   /* スキャンラインが高さと等しい場合は処理をスキップ */
   ...
   /* Check buffer line validity */
   if (buf == NULL) {
      ESP_LOGE(TAG, "Invalid buffer line (NULL) for scanline %d in ppu_renderscanline", scanline);
      return;
   }
...
{
   /* スキャンラインの範囲チェック */
   if (scanline < 0 || scanline > 261) {
      ESP_LOGE(TAG, "Invalid scanline in ppu_endscanline: %d", scanline);
      return;
   }
...
void ppu_scanline(bitmap_t *bmp, int scanline, bool draw_flag)
{
   ...
   if (bmp == NULL) {
      ESP_LOGE(TAG, "Invalid bitmap (NULL) in ppu_scanline");
      return;
   }

   /* NESの標準的なスキャンライン範囲は0-261 */
   if (scanline < 0 || scanline > 261) {
      ESP_LOGE(TAG, "Scanline out of valid range in ppu_scanline: %d", scanline);
      return;
   }

   /* デバッグ情報: 500フレームごとにスキャンライン情報をログ出力 */
   static int frame_counter = 0;
   if (scanline == 0) {
      frame_counter++;
      if (frame_counter % 500 == 0) {
         ESP_LOGI(TAG, "Frame: %d, bitmap height: %d", frame_counter, bmp->height);
         
         /* ビットマップの最初と最後の数行のラインポインタをチェック */
         for (int i = 0; i < 5 && i < bmp->height; i++) {
            ESP_LOGI(TAG, "Line[%d] pointer: %p", i, bmp->line[i]);
         }
         for (int i = bmp->height - 5; i < bmp->height && i >= 0; i++) {
            ESP_LOGI(TAG, "Line[%d] pointer: %p", i, bmp->line[i]);
         }
      }
   }
   ...
   if (scanline < 240)
   {
      ...
      /* 上下のはみ出る部分をカットして描画するための計算 */
      /* NESの表示可能なスキャンラインは0-239、ビットマップの高さは224 */
      
      /* 上下に合計 16 ラインの余白を作るため、上下に8ラインずつカットする */
      const int top_margin = 8;     /* 上部のカットするライン数 */
      const int visible_lines = 224; /* 表示するライン数 */
      
      /* 上部のカットを考慮して、表示範囲内のスキャンラインのみ描画 */
      if (scanline >= top_margin && scanline < (top_margin + visible_lines)) {
         /* カットした分を考慮してスキャンラインを調整 */
         int adjusted_scanline = scanline - top_margin;
         
         /* 重要なスキャンラインの場合、詳細なデバッグ情報を出力 */
         if (frame_counter % 500 == 0 && 
             (scanline == top_margin || scanline == top_margin + visible_lines - 1 || scanline % 60 == 0)) {
            ESP_LOGI(TAG, "Scanline: %d -> adjusted: %d (with margins)", scanline, adjusted_scanline);
         }
         
         /* スキャンラインがビットマップの高さ以内の場合のみレンダリングを行う */
         if (adjusted_scanline >= 0 && adjusted_scanline < bmp->height) {
            /* 明示的にスキャンラインを設定してレンダリング */
            ppu_renderscanline(bmp, adjusted_scanline, draw_flag);
            
            /* 最初と最後のスキャンラインの場合、追加のデバッグ情報を出力 */
            if (frame_counter % 500 == 0 && (adjusted_scanline == 0 || adjusted_scanline == bmp->height - 1)) {
               ESP_LOGI(TAG, "Rendering to scanline: %d, buffer addr: %p", 
                        adjusted_scanline, bmp->line[adjusted_scanline]);
            }
         } else {
            /* 範囲外のスキャンラインの場合は警告を出力 */
            if (frame_counter % 500 == 0 && (scanline % 60 == 0)) {
               ESP_LOGW(TAG, "Skipping out-of-bounds scanline: %d -> adjusted: %d (height: %d)", 
                        scanline, adjusted_scanline, bmp->height);
            }
         }
      } else {
         /* 表示範囲外のスキャンラインの場合 */
         if (frame_counter % 500 == 0 && scanline % 60 == 0) {
            ESP_LOGD(TAG, "Scanline %d outside visible range [%d-%d]", 
                     scanline, top_margin, top_margin + visible_lines - 1);
         }
      }
   }
...
INLINE void draw_box(bitmap_t *bmp, int x, int y, int height)
{
   ...
   /* Check bitmap validity */
   if (bmp == NULL || y < 0 || y >= bmp->height) {
      ESP_LOGE(TAG, "Invalid bitmap or y-coordinate in draw_box: %p, %d", bmp, y);
      return;
   }

   /* Check line pointer validity */
   if (bmp->line[y] == NULL) {
      ESP_LOGE(TAG, "Invalid line pointer in draw_box at y=%d", y);
      return;
   }
...
INLINE void draw_deadsprite(bitmap_t *bmp, int x, int y, int height)
{
   ...
   /* Check bitmap validity */
   if (bmp == NULL || y < 0 || y >= bmp->height) {
      ESP_LOGE(TAG, "Invalid bitmap or y-coordinate in draw_deadsprite: %p, %d", bmp, y);
      return;
   }

   /* Check line pointer validity */
   if (bmp->line[y] == NULL) {
      ESP_LOGE(TAG, "Invalid line pointer in draw_deadsprite at y=%d", y);
      return;
   }
   ...
   for (i = 0; i < height; i++)
   {
      /* Check if we're going beyond bitmap height */
      if (y + i >= bmp->height) {
         ESP_LOGE(TAG, "Attempt to draw beyond bitmap height in draw_deadsprite: %d", y + i);
         break;
      }
