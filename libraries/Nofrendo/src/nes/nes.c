...
static void nes_renderframe(bool draw_flag)
{
   ...
   if (nes.scanline == 0) {
      ...
      if (frame_counter % 500 == 0) {
         ESP_LOGI("NES", "Frame: %d - Resetting task watchdog", frame_counter);
      }
   }
...
//      ppu_scanline(nes.vidbuf, nes.scanline, draw_flag);
      ...
      // 一定間隔でタスクウォッチドッグをリセット
      if (nes.scanline % 60 == 0) {
         esp_task_wdt_reset();
      }
...
void nes_emulate(void)
{
   ...
   {
      // タスクウォッチドッグをリセット（ESP-IDF用）
      #ifdef ESP_PLATFORM
      esp_err_t wdt_err = esp_task_wdt_reset();
      if (wdt_err != ESP_OK) {
          ESP_LOGW("NES", "Failed to reset task watchdog: %d", wdt_err);
      }
      #endif
      ...
      else if (frames_to_render > 1)
      {
         ...
         // フレーム描画時にタスクウォッチドッグをリセット（ESP-IDF用）
         #ifdef ESP_PLATFORM
         esp_task_wdt_reset();
         #endif
      }
